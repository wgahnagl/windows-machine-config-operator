...
File a GitHub issue and attach the logs to the issue along with the _MachineSet_ used.
...
- The key used in the _cloud-private-key_ [secret](../README.md#Usage) and the key used when creating the cluster should
...
- _\<username\>_ is _Administrator_ (AWS) or _capi_ (Azure)
- _\<windows-node-internal-ip\>_ is the internal IP address of the node, which can be discovered by:
  ```shell script
...
- SSH into the Windows node and execute the following command to create a password for the user:
  ...
  ```
- You can now RDP into the Windows node at _localhost:2020_ using an RDP client
...
Kubernetes node log files are in _C:\var\logs_. To view all the directories under _C:\var\logs_, execute:
